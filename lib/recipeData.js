...
const recipeDb = async () => {
    ...
    console.log(`  Count: 
        landing_pages:{ ${maps.landing_pages.length} }
        affiliate_websites:{ ${Object.keys(maps.affiliate_websites).length} }
        c_group_segment:{ ${Object.keys(maps.c_group_segment).length} }
        c_group:{ ${Object.keys(maps.c_group).length} }
        adUnits:{ ${Object.keys(maps.adUnits).length} }
        segments:{ ${Object.keys(maps.segments).length} }
        aff_info:{ ${Object.keys(maps.aff_info).length} }
        campaigns:{ ${Object.keys(maps.campaigns).length} }
        dimensions:{ ${Object.keys(maps.dimensions).length} }
        smart_ad:{ ${Object.keys(maps.smart_ad).length} }
        recipeDb:{ ${recipeDb.length} }`
    )
